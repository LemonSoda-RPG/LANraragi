...
sub translate_tag_to_cn ( $list, $db_path ) {
    ...
    unless ( defined $db_path && length $db_path ) {
        $logger->warn("ETagCN translation database path is not configured; keeping original tags.");
        return $list;
    }
    ...
    if ( $@ || ref($json) ne "HASH" || ref( $json->{data} ) ne "ARRAY" ) {
        $logger->warn("Invalid ETagCN translation database '$db_path'; keeping original tags.");
        return $list;
    }
    ...
    for my $item (@$list) {
        ...
        my ( $namespace, $key ) = split( /:/, $item, 2 );
        next unless defined $namespace;

        for my $element (@$target) {
            next unless ref($element) eq "HASH";
            next unless ( $element->{namespace} // "" ) eq $namespace;

            my $front_matters = ref( $element->{frontMatters} ) eq "HASH"
              ? $element->{frontMatters}
              : {};
            my $translated_namespace = $front_matters->{name} // $namespace;
            my $translated_key       = $key;
            my $data                 = ref( $element->{data} ) eq "HASH" ? $element->{data} : {};

            if ( defined $key && ref( $data->{$key} ) eq "HASH" ) {
                $translated_key = $data->{$key}{name} // $key;
            }

            $item = defined $key
              ? "$translated_namespace:$translated_key"
